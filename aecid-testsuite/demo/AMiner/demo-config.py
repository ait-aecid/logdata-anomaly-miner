...
# Build the parsing model:
  ...
  service_children_cron_job_announcement.append(FixedDataModelElement('StartTime', b'\' in 5 min.'))
...
# or User 'username' (logged in, logged out) x minutes ago.
  ...
  whitelist_violation_detector = WhitelistViolationDetector(analysis_context.aminer_config, whitelist_rules, anomaly_event_handlers, output_log_line=True)
  ...
  new_match_path_detector = NewMatchPathDetector(
      analysis_context.aminer_config, anomaly_event_handlers, auto_include_flag=True, output_log_line=True)
  ...
  histogram_analysis = HistogramAnalysis(analysis_context.aminer_config, [('/model/RandomTime/Random', modulo_time_bin_definition), ('/model/Random', linear_numeric_bin_definition)],
          10, anomaly_event_handlers, output_log_line=True)
  ...
  path_dependent_histogram_analysis = PathDependentHistogramAnalysis(analysis_context.aminer_config, '/model/RandomTime', modulo_time_bin_definition, 10, anomaly_event_handlers, output_log_line=True)
  ...
  match_value_average_change_detector = MatchValueAverageChangeDetector(analysis_context.aminer_config, anomaly_event_handlers, None, ['/model/Random'], 100, 10, output_log_line=True)
  ...
  missing_match_path_value_detector = MissingMatchPathValueDetector(
        analysis_context.aminer_config, '/model/DiskReport/Space', anomaly_event_handlers, auto_include_flag=True, default_interval=2, realert_interval=5, output_log_line=True)
  ...
  time_correlation_detector = TimeCorrelationDetector(analysis_context.aminer_config, 2, 1, 0, anomaly_event_handlers, record_count_before_event=3000, output_log_line=True)
