...
class EventCorrelationDetector(AtomHandlerInterface, TimeTriggeredComponentInterface):
    ...
    def __init__(self, aminer_config, anomaly_event_handlers, paths=None, max_hypotheses=1000, hypothesis_max_delta_time=5.0,
                 ...
                 auto_include_flag=True, passlisted_paths=None, persistence_id='Default', output_log_line=True):
        ...
        @param passlisted_paths list of paths that are not considered for correlation, i.e., events that contain one of these paths are
        ...
        self.passlisted_paths = passlisted_paths
        if self.passlisted_paths is None:
            self.passlisted_paths = []
    ...
    def receive_atom(self, log_atom):
        ...
        if self.paths is None or len(self.paths) == 0:
            ...
            # Skip passlisted paths.
            for passlisted in self.passlisted_paths:
                if passlisted in log_event:
                    return
        else:
