...
# ToDo:
# x) Remove unwanted dependency on packages (TODOs in pmdarima, but verry intervined)
# x) Persistency
# x) Cleanup
...
class TSAArima(AtomHandlerInterface, TimeTriggeredComponentInterface, EventSourceInterface):
    ...
    def __init__(self, aminer_config, anomaly_event_handlers, event_type_detector, num_division_time_step=10, alpha=0.05,
                 persistence_id='Default', output_log_line=True, ignore_list=None, constraint_list=None):
        ...
        # List of the the single arima_models (pdmarima)
        self.arima_models_pdmarima = []
    ...
    def calculate_time_steps(self, counts, log_atom):
        ...
        self.arima_models_pdmarima = [None for _ in range(len(counts))]
    ...
    def test_num_appearance(self, event_number, count, current_time, log_atom):
        ...
        if len(self.time_window_history) <= event_number:
            ...
            self.arima_models_pdmarima += [None for _ in range(event_number + 1 - len(self.arima_models_pdmarima))]
        ...
        if self.arima_models_pdmarima[event_number] == None:
            ...
            if len(self.time_window_history[event_number]) >= 10*self.num_division_time_step:
                ...
                if not self.build_sum_TSA:
                    ...
                    try:
                        ...
                        self.arima_models_pdmarima[event_number] = model.fit(self.time_window_history[event_number][-10*self.num_division_time_step:])
                        ...
                        self.arima_models_pdmarima[event_number] = None
                    ...
                    try:
                        ...
                        self.arima_models_pdmarima[event_number] = model.fit([sum(self.time_window_history[event_number][-10*self.num_division_time_step+i:-9*self.num_division_time_step+i]) for i in range(9*self.num_division_time_step)]+[sum(self.time_window_history[event_number][-self.num_division_time_step:])])
                    except:
                        self.arima_models_pdmarima[event_number] = None
        # Add the new value and make a one step prediction
        else:
            if not self.build_sum_TSA:
                pred, lower_limit, upper_limit, lower_limit2, upper_limit2 = self.one_step_prediction(event_number)
                self.plots[0].append(lower_limit)
                self.plots[1].append(count)
                self.plots[2].append(upper_limit)
                self.plots[3].append(lower_limit2)
                self.plots[4].append(upper_limit2)
                self.time_history.append(current_time)

                # Test if count is in boundaries
                message = 'EventNumber: %s, Count: %s, Lower: %s, Pred: %s, Upper: %s'%(event_number, count, lower_limit, pred, upper_limit)
                #print(message)
                ...
                self.arima_models_pdmarima[event_number].update([count])
                ...
                pred, lower_limit, upper_limit = self.one_step_prediction(event_number)
                self.plots[0].append(lower_limit)
                ...
                message = 'EventNumber: %s, Count: %s, Lower: %s, Pred: %s, Upper: %s'%(event_number, count_sum, lower_limit, pred, upper_limit)
                #print(message)
                ...
                # Update the model, for the next step
                self.arima_models_pdmarima[event_number].update([count_sum])

    def one_step_prediction(self, event_number):
        prediction = self.arima_models_pdmarima[event_number].predict(n_periods=1, return_conf_int=True, alpha=self.alpha)
        prediction2 = self.arima_models_statsmodels[event_number].get_forecast(1)
        prediction2 = prediction2.conf_int(alpha=self.alpha)
        # return in the order: pred, lower_limit, upper_limit
        return prediction[0][0], prediction[1][0][0], prediction[1][0][1], prediction2[0][0], prediction2[0][1]
