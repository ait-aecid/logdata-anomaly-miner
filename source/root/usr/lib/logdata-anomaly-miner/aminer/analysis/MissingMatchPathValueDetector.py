...
class MissingMatchPathValueDetector(AtomHandlerInterface, TimeTriggeredComponentInterface, EventSourceInterface):
    ...
    def passlist_event(self, event_type, sorted_log_lines, event_data, passlisting_data):
        ...
        @return a message with information about passlisting using given passlisting_data was not possible."""
        if event_type != self.analysis_string % self.__class__.__name__:
            raise Exception('Event not from this source')
        if not isinstance(passlisting_data, int):
            raise Exception('Passlisting data has to integer with new interval, -1 to reset to defaults, other negative '
                            'value to remove the entry')
        new_interval = passlisting_data
        if new_interval == -1:
