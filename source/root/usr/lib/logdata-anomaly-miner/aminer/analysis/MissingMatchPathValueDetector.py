"""This module provides the MissingMatchPathValueDetector to generate events
when expected values were not seen for an extended period of time.
...
class MissingMatchPathValueDetector(
    ...
    """This class creates events when an expected value is not seen within a
    given timespan.
    ...
    def check_timeouts(self, timestamp, log_atom):
        """Check if there was any timeout on a channel, thus triggering event
        dispatching."""
...
class MissingMatchPathListValueDetector(MissingMatchPathValueDetector):
    ...
    It only can look up values from a list of paths until one path
    really exists. It then uses this value as key to detect log atoms
    belonging to the same data stream. This is useful when e.g. due to
    different log formats, the hostname, servicename or any other
