...
class NewMatchPathValueComboDetector(AtomHandlerInterface, TimeTriggeredComponentInterface, EventSourceInterface):
    ...
    def passlist_event(self, event_type, sorted_log_lines, event_data, passlisting_data):
        """Passlist an event generated by this source using the information emitted when generating the event.
        @return a message with information about passlisting
        @throws Exception when passlisting of this special event using given passlisting_data was not possible."""
        if event_type != 'Analysis.%s' % self.__class__.__name__:
            raise Exception('Event not from this source')
        if passlisting_data is not None:
            raise Exception('Passlisting data not understood by this detector')
        self.known_values_set.add(event_data[1])
        return 'Passlisted path(es) %s with %s in %s' % (', '.join(self.target_path_list), event_data[1], sorted_log_lines[0])
