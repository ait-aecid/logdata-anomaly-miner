...
class JsonModelElement(ModelElementInterface):
    ...
    def get_match_element(self, path: str, match_context):
        ...
        try:
            ...
            while index != -1:
                ...
                if index != -1:
                    try:
                        ...
                        break
            index = 0
            quotes = [i for i in range(len(match_context.match_data)) if match_context.match_data.startswith(b"\"", i)]
            while index != -1:
            ...
            for num in b"\n\r\t":
                ...
                while index != -1:
                    ...
                    if index != -1 and len(match_context.match_data) - 1 > index and self.index_is_in_json_string(quotes, index):
                        escaped = (bytes([match_context.match_data[index]]).replace(b"\\", b"\\\\").replace(b"\a", b"\\a")
                                   .replace(b"\f", b"\\f").replace(b"\n", b"\\n").replace(b"\r", b"\\r")
                                   .replace(b"\t", b"\\t").replace(b"\v", b"\\v"))
            ...
            logging.getLogger(DEBUG_LOG_NAME).debug("DDDDDDDDD")
