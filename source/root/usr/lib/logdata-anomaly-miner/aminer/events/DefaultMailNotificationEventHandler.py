...
events and send them via "sendmail" transport."""
  ...
  def __init__(self, analysis_context):
    ...
    aminerConfig = analysis_context.aminer_config
    self.recipient_address = aminerConfig.configProperties.get(
...
# Avoid too many calls to the operating system time()
    ...
    if self.events_collected < self.max_events_per_message:
      ...
      self.current_message += self.event_data.receiveEventString()
