...
class KafkaEventHandler():
  ...
  forward Json-objects to a Kafka queue."""
  ...
  def receive_event(self, event_type, event_message, sorted_log_lines, json_dump, log_atom,
                    event_source):
    ...
    if not isinstance(json_dump, str) and not isinstance(json_dump, bytes):
      print('WARNING: KafkaEventHandler received non-string event data. Use the JsonConverterHandler to serialize it first.', file=sys.stderr)
      return False
    try:
