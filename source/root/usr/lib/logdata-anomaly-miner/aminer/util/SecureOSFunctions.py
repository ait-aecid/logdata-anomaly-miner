...
def send_annotated_file_descriptor(send_socket, send_fd, type_info, annotation_data):
  ...
  @param type_info has to be a null-byte free string to inform
  the receiver how to handle the file descriptor and how to interpret
  the annotationData.
  @param annotation_data this optional byte array  may convey
...
# Construct the message data first
  ...
  send_socket.sendmsg(
      [message_data],
      [(socket.SOL_SOCKET, socket.SCM_RIGHTS, struct.pack('i', send_fd))])
...
def receive_annoted_file_descriptor(receive_socket):
  ...
  if len(anc_data) != 1:
    raise Exception(
        'Received %d sets of ancillary data instead of 1' % len(anc_data))
